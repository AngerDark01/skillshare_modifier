...
func TestBasePath_IndexHtmlInjection(t *testing.T) {
	...
	if !strings.Contains(body, `<base href="/app/">`) {
		t.Errorf("expected base href injection, got:\n%s", body)
	}
...
<html>
  ...
  <body></body>
	...
	if !strings.Contains(body, `<base href="/">`) {
		t.Errorf("expected root base href injection, got:\n%s", body)
	}
...
func TestBasePath_IndexHtmlInjectionWithHeadAttributes(t *testing.T) {
	tmp := t.TempDir()
	indexHTML := `<!DOCTYPE html><html><head data-app="skillshare"><title>Test</title></head><body></body></html>`
	os.WriteFile(filepath.Join(tmp, "index.html"), []byte(indexHTML), 0644)
	...
	body := rr.Body.String()
	if !strings.Contains(body, `<head data-app="skillshare"><base href="/app/">`) {
		t.Errorf("expected base href after attributed head tag, got:\n%s", body)
	}
}
...
func TestBasePath_SPAFallbackWithInjection(t *testing.T) {
	...
	body := rr.Body.String()
	if !strings.Contains(body, `<base href="/myapp/">`) {
		t.Errorf("SPA fallback should inject base href, got:\n%s", body)
	}
