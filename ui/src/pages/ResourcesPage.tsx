...
/** Build a nested folder tree from skills' relPath values and explicit empty folders. Computes skillCount bottom-up. */
function buildTree(skills: Skill[], folders: string[] = []): FolderNode {
  const root: FolderNode = { name: '', path: '', children: new Map(), skills: [], skillCount: 0, targetSummary: defaultTargetSummary };
  for (const folder of folders) {
    if (!folder) continue;
    ensureFolderNode(root, folder).explicit = true;
  }
  for (const skill of skills) {
    ...
    const node = ensureFolderNode(root, dirPath);
...
function flattenTree(
  ...
  const shouldShowFolder = (node: FolderNode) => node.skillCount > 0 || (!!node.explicit && !isSearching);
  ...
  function walkFolder(node: FolderNode, depth: number) {
    ...
    for (const [, child] of sortedChildren) {
      ...
      if (!shouldShowFolder(child)) continue; // skip empty folders while filtering
      result.push({
        type: 'folder',
        name: child.name,
        path: child.path,
  ...
  for (const [, child] of sortedChildren) {
    const cc = child.skillCount;
    if (!shouldShowFolder(child)) continue;
    result.push({
      type: 'folder',
      name: child.name,
      path: child.path,
...
function folderFromSkillRelPath(relPath: string): string {
...
function FolderPathDialog({
  ...
  return (
    <DialogShell open={true} onClose={onCancel} preventClose={loading}>
      <form
        onSubmit={(e) => {
          e.preventDefault();
      ...
      >
        <h3 className="text-lg font-bold text-pencil mb-4">{title}</h3>
        <Input
          label={t('resources.folder.folderPath')}
          value={folder}
          onChange={(e) => setFolder(e.target.value)}
          placeholder="02-code-quality/review"
          autoFocus
        />
        <div className="flex gap-3 justify-end mt-6">
          ...
          <Button type="submit" variant="primary" size="md" loading={loading} disabled={!normalized}>
            {confirmText}
          </Button>
        </div>
      </form>
...
function MoveSkillDialog({
  ...
  const t = useT();
  const currentFolder = folderFromSkillRelPath(skill.relPath);
  const [folder, setFolder] = useState(currentFolder);

  const options = useMemo<SelectOption[]>(() => {
    const all = new Set<string>(['', ...folders]);
    if (currentFolder) all.add(currentFolder);
    return [...all].sort((a, b) => {
      if (a === '') return -1;
      if (b === '') return 1;
      ...
      value,
      label: value || t('resources.folder.root'),
    }));
  }, [folders, currentFolder, t]);
  ...
  return (
    <DialogShell open={true} onClose={onCancel} preventClose={loading}>
      ...
      >
        <h3 className="text-lg font-bold text-pencil mb-2">{t('resources.folder.moveSkillTitle')}</h3>
        <p className="text-pencil-light mb-4">
          <strong className="text-pencil">{skill.name}</strong>
        </p>
        <div className="space-y-3">
          <Select
            label={t('resources.folder.existingFolder')}
            value={options.some((o) => o.value === folder) ? folder : ''}
            onChange={setFolder}
            options={options}
          />
          <Input
...
export default function SkillsPage() {
  ...
  const urlActiveTab: ResourceTab | null = urlTab === 'agents' || urlTab === 'skills' ? urlTab : null;
  const [savedActiveTab, setSavedActiveTab] = useState<ResourceTab>(() => {
    const saved = localStorage.getItem('skillshare:resources-tab');
    return saved === 'agents' ? 'agents' : 'skills';
  });
  const activeTab = urlActiveTab ?? savedActiveTab;
...
const INDENT_PX = 24;
const DRAG_SKILL_MIME = 'application/x-skillshare-skill';
...
function FolderTreeView({ skills, resourceKind, totalCount, isSearching, stickyTop = 0, onClearFilters }: {
  ...
  const [folderDialog, setFolderDialog] = useState<{ initialFolder: string } | null>(null);
  const [moveSkill, setMoveSkill] = useState<Skill | null>(null);
  ...
  const createFolderMutation = useMutation({
    mutationFn: (folder: string) => api.createResourceFolder(folder),
    onSuccess: (data) => {
      toast(t('resources.folder.created', { folder: data.folder }), 'success');
      setFolderDialog(null);
    },
    onError: (err: Error) => toast(err.message, 'error'),
    onSettled: () => queryClient.invalidateQueries({ queryKey: queryKeys.skills.folders }),
  });

  const moveMutation = useMutation({
    mutationFn: ({ name, folder }: { name: string; folder: string }) => api.moveResource(name, folder),
    onSuccess: (data) => {
      toast(t('resources.folder.moved', { name: data.name, folder: folderFromSkillRelPath(data.to) || t('resources.folder.root') }), 'success');
      setMoveSkill(null);
    },
    onError: (err: Error) => toast(err.message, 'error'),
    onSettled: () => {
  ...
  const tree = useMemo(() => buildTree(skills, resourceKind === 'skill' ? folders : []), [skills, folders, resourceKind]);
  ...
  const openNewFolderDialog = useCallback((parentPath = '') => {
    const initialFolder = parentPath ? `${parentPath}/` : '';
    setFolderDialog({ initialFolder });
    setContextMenu(null);
  }, []);

  const startMoveSkill = useCallback((skill: Skill) => {
    setMoveSkill(skill);
    setContextMenu(null);
  }, []);

  const dropSkillIntoFolder = useCallback((folderPath: string, e: React.DragEvent) => {
    e.preventDefault();
    setDragOverFolder(null);
    if (resourceKind !== 'skill' || moveMutation.isPending) return;
    const raw = e.dataTransfer.getData(DRAG_SKILL_MIME);
    if (!raw) return;
    try {
      const payload = JSON.parse(raw) as Pick<Skill, 'flatName' | 'relPath' | 'isInRepo' | 'kind'>;
      if (payload.kind !== 'skill' || payload.isInRepo) return;
      if (folderFromSkillRelPath(payload.relPath) === folderPath) return;
      moveMutation.mutate({ name: payload.flatName, folder: folderPath });
    } catch {
      // Ignore malformed drag payloads from outside the app.
    }
  }, [moveMutation, resourceKind]);
  ...
  const renderItem = useCallback((index: number): ReactElement => {
    ...
    if (node.type === 'folder') {
      ...
      return (
        <div
          data-tree-idx={index}
          className={`relative flex items-center gap-1.5 py-1.5 px-1 cursor-pointer select-none hover:bg-muted/50 transition-colors${node.isRoot ? ' border-t border-muted/60 mt-2 pt-3' : ''}${contextMenu?.mode === 'folder' && contextMenu.folderPath === node.path ? ' bg-muted/50' : ''}${dragOverFolder === node.path ? ' bg-muted/70 outline outline-2 outline-pencil/25' : ''}`}
          style={{ paddingLeft: node.depth * INDENT_PX + 4 }}
          onClick={() => toggleFolder(node.path)}
          onDragOver={(e) => {
            if (resourceKind !== 'skill' || moveMutation.isPending) return;
            if (!Array.from(e.dataTransfer.types).includes(DRAG_SKILL_MIME)) return;
            e.preventDefault();
            setDragOverFolder(node.path);
          }}
          onDragLeave={() => setDragOverFolder((current) => current === node.path ? null : current)}
          onDrop={(e) => dropSkillIntoFolder(node.path, e)}
          onContextMenu={(e) => {
            e.preventDefault();
    ...
    const canMoveSkill = resourceKind === 'skill' && skill.kind === 'skill' && !skill.isInRepo;
    ...
    return (
      ...
      >
        <Tooltip content={tooltipContent} followCursor delay={1000}>
          <Link
            to={resourceDetailHref(skill)}
            draggable={canMoveSkill}
            onDragStart={(e) => {
              if (!canMoveSkill) return;
              e.dataTransfer.effectAllowed = 'move';
              e.dataTransfer.setData(DRAG_SKILL_MIME, JSON.stringify({
            ...
            className={`relative flex items-center gap-1.5 py-1 px-1 hover:bg-muted/50 transition-colors no-underline${canMoveSkill ? ' cursor-grab active:cursor-grabbing' : ''}${skill.disabled ? ' opacity-40' : ''}${contextMenu?.mode === 'skill' && contextMenu.skillFlatName === skill.flatName ? ' bg-muted/50' : ''}`}
            style={{ paddingLeft: node.depth * INDENT_PX + 4 }}
          >
            {indentGuides}
  ...
  }, [rows, collapsed, isSearching, toggleFolder, contextMenu, pendingFolder, resourceKind, batchMutation.isPending, dragOverFolder, moveMutation.isPending, dropSkillIntoFolder, treeGetSkillTargets, t]);
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 mb-3 flex-wrap">
        ...
        {(resourceKind === 'skill' || folderCount > 1) && (
          <span className="ml-auto flex items-center gap-2">
            {resourceKind === 'skill' && !isSearching && (
              <Button variant="ghost" size="sm" onClick={() => openNewFolderDialog('')} disabled={createFolderMutation.isPending}>
                <Plus size={14} strokeWidth={2.5} /> {t('resources.folder.newFolder')}
              </Button>
            )}
            {folderCount > 1 && (
              <>
      ...
      {contextMenu && (
        <TargetMenu
          ...
          extraItems={contextMenu.mode === 'skill' ? [
            ...(((contextMenu.kind ?? resourceKind) === 'skill' && !contextMenu.isInRepo ? [{
              key: 'move',
              label: t('resources.contextMenu.moveToFolder'),
              icon: <FolderOpen size={13} strokeWidth={2.5} />,
              onSelect: () => startMoveSkill({
            ...
            {
              key: 'new-subfolder',
              label: t('resources.contextMenu.newSubfolder'),
              icon: <Plus size={13} strokeWidth={2.5} />,
              onSelect: () => openNewFolderDialog(contextMenu.folderPath ?? ''),
            },
          ]}
          onSelect={(target) => {
            if (batchMutation.isPending) return;
      ...
      {folderDialog && (
        <FolderPathDialog
          key={folderDialog.initialFolder}
          initialFolder={folderDialog.initialFolder}
          title={t('resources.folder.newFolderTitle')}
          confirmText={t('resources.folder.create')}
          loading={createFolderMutation.isPending}
          onConfirm={(folder) => createFolderMutation.mutate(folder)}
          onCancel={() => setFolderDialog(null)}
        />
      )}
      {moveSkill && (
        <MoveSkillDialog
          key={moveSkill.flatName}
          skill={moveSkill}
          folders={folders}
          loading={moveMutation.isPending}
          onConfirm={(folder) => moveMutation.mutate({ name: moveSkill.flatName, folder })}
          onCancel={() => setMoveSkill(null)}
        />
      )}
      <ConfirmDialog
